...
const AdminResources: React.FC = () => {
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            if (!res.ok) {
                ...
                throw new Error(errData.message || 'Erreur lors du téléversement (Fichier trop gros ?)');
            }
    ...
    return (
        <App breadcrumb="Administration" title="Ressources 3D">
            ...
            <div style={{ display: 'flex', gap: '8px', marginBottom: '32px', background: 'rgba(0,0,0,0.02)', padding: '6px', borderRadius: '12px', width: 'fit-content', border: '1px solid var(--dash-border)' }}>
                ...
                >
                    <Database size={16} /> Modèles existants
                </button>
                <button
                ...
                >
                    <UploadCloud size={16} /> Importer un modèle
                </button>
            </div>
            ...
            {activeSection === 'list' && (
                <div style={{ background: 'var(--dash-bg)', border: '1px solid var(--dash-border)', borderRadius: '16px', overflow: 'hidden' }}>
                    <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--dash-border)', fontWeight: 700, display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <Box size={18} color="#0ea5e9"/> Liste des modèles
                    </div>
                        ...
                        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                            <thead>
                                <tr style={{ background: 'rgba(0,0,0,0.02)', borderBottom: '1px solid var(--dash-border)', fontSize: '11px', textTransform: 'uppercase', color: 'var(--dash-text-muted)' }}>
                                    <th style={{ padding: '12px 20px', textAlign: 'left' }}>Nom / Fichier</th>
                                    <th style={{ padding: '12px 20px', textAlign: 'center' }}>Objets</th>
                                    <th style={{ padding: '12px 20px', textAlign: 'right' }}>Date</th>
                                </tr>
                            </thead>
                            <tbody>
                                {loading ? (
                                    <tr><td colSpan={3} style={{ padding: '40px', textAlign: 'center' }}>Chargement...</td></tr>
                                ) : assets.length === 0 ? (
                                    <tr><td colSpan={3} style={{ padding: '40px', textAlign: 'center' }}>Aucun modèle.</td></tr>
                                ) : (
                                    assets.map(a => (
                ...
                <div style={{ maxWidth: '800px', margin: '0 auto' }}>
                    ...
                    {uploadPhase === 'done' ? (
                        <div style={{ textAlign: 'center', padding: '60px', background: 'var(--dash-bg)', borderRadius: '24px', border: '1px solid var(--dash-border)' }}>
                            ...
                            <h3 style={{ fontSize: '24px', marginBottom: '8px' }}>Importation réussie !</h3>
                            <p style={{ color: 'var(--dash-text-muted)' }}>Le modèle a été ajouté à votre bibliothèque.</p>
                        </div>
                        ...
                        <div style={{ textAlign: 'center', padding: '100px', background: 'var(--dash-bg)', borderRadius: '24px', border: '1px solid var(--dash-border)' }}>
                            ...
                            <p style={{ color: 'var(--dash-text-muted)' }}>Veuillez patienter pendant le traitement du fichier.</p>
                        </div>
                    ) : (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                            ...
                            {uploadPhase === 'idle' && (
                                <div style={{ background: 'var(--dash-bg)', padding: '60px 40px', borderRadius: '24px', border: '2px dashed #f59e0b40', textAlign: 'center', transition: '0.3s' }} className="upload-zone">
                                    ...
                                    <h2 style={{ marginBottom: '12px', fontSize: '22px' }}>Importer depuis votre appareil</h2>
                                    <p style={{ fontSize: '15px', color: 'var(--dash-text-muted)', marginBottom: '32px', maxWidth: '400px', margin: '0 auto 32px' }}>
                                        Sélectionnez un fichier 3D au format <b>.glb</b> (ou une archive <b>.zip</b> contenant modèle et données).
                                    </p>
                                    ...
                                    <label style={{ display: 'inline-flex', padding: '14px 32px', background: '#f59e0b', color: '#fff', borderRadius: '12px', fontWeight: 700, cursor: 'pointer', gap: '10px', fontSize: '16px', boxShadow: '0 8px 20px rgba(245, 158, 11, 0.2)' }}>
                                        <UploadCloud size={20} /> Sélectionner le fichier
                            ...
                            {(uploadPhase === 'preview' || uploadPhase === 'saving') && (
                                <div style={{ background: '#0ea5e908', padding: '40px', borderRadius: '24px', border: '2px solid #0ea5e9' }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '32px' }}>
                                        <div>
                                            <h3 style={{ margin: 0, fontSize: '20px' }}>Finaliser l'importation</h3>
                                            <p style={{ fontSize: '14px', color: 'var(--dash-text-muted)', marginTop: '4px' }}>{previewData?.filename}</p>
                                        </div>
                                        <div style={{ display: 'flex', gap: '12px' }}>
                                            <button onClick={resetUpload} style={{ padding: '10px 20px', borderRadius: '10px', border: '1px solid var(--dash-border)', background: 'var(--dash-bg)', color: 'var(--dash-text)', fontWeight: 600, cursor: 'pointer' }}>Annuler</button>
                                            <button onClick={handleConfirm} disabled={uploadPhase === 'saving' || !assetName} style={{ padding: '10px 24px', background: '#34d399', color: '#fff', border: 'none', borderRadius: '10px', fontWeight: 700, cursor: 'pointer', boxShadow: '0 4px 12px rgba(52, 211, 153, 0.3)' }}>
                                                {uploadPhase === 'saving' ? 'Enregistrement...' : 'Valider et Ajouter'}
                                            </button>
                                        </div>
                                    </div>
                                    
                                    <div style={{ marginBottom: '24px' }}>
                                        <label style={{ display: 'block', fontSize: '12px', fontWeight: 700, marginBottom: '8px', textTransform: 'uppercase', color: 'var(--dash-text-muted)', letterSpacing: '0.05em' }}>Nom d'affichage dans la bibliothèque</label>
                                    ...
                                    <div>
                                        <label style={{ display: 'block', fontSize: '12px', fontWeight: 700, marginBottom: '12px', textTransform: 'uppercase', color: 'var(--dash-text-muted)', letterSpacing: '0.05em' }}>Objets anatomiques détectés ({previewObjects.length})</label>
                                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', maxHeight: '250px', overflowY: 'auto', padding: '4px' }}>
                                            {previewObjects.length > 0 ? previewObjects.map((obj, i) => (
                                                ...
                                                <div style={{ gridColumn: '1/-1', textAlign: 'center', padding: '20px', color: 'var(--dash-text-muted)', fontSize: '13px' }}>Aucun objet automatique. Vous pourrez les ajouter manuellement plus tard.</div>
                                            )}
                                        </div>
                                    </div>
                                </div>
